...
class IconSearchService {
    ...
    func searchIcons(query: String, completion: @escaping ([IconModel]?, Error?) -> Void) {
        ...
        let queryItems: [URLQueryItem] = [
            ...
            URLQueryItem(name: "count", value: "\(resultsPerPage)"),
            URLQueryItem(name: "type", value: "png"),
            URLQueryItem(name: "premium", value: "false"),
            URLQueryItem(name: "offset", value: "\(currentPage * resultsPerPage)")
        ]
    ...
    func loadNextPage(query: String, completion: @escaping ([IconModel]?, Error?) -> Void) {
        currentPage += 1
        searchIcons(query: query, completion: completion)
    }
}
